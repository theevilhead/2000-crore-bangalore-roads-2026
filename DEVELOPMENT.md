...
- `lib/geo/snap.ts` - snaps clicked waypoints to road centerlines (Directions API,
  walking profile so one-way / turn rules are ignored).
- `supabase/migrations/20260607000100_init.sql` - `reports`, `corroborations`, and RPCs:
...
- Ward / constituency derivation (`20260607000200_wards.sql`) is wired but inert
  until BBMP ward boundaries are imported into the `wards` table (phase-2
  instrumentation).
